/**
 * Data Types & Fallback Data
 *
 * TypeScript interfaces for products, categories, banners, etc.
 *
 * Primary data source: Firestore collections (via hooks/).
 * Fallback data below is used whenever Firestore returns an
 * empty result (no project configured, offline, etc.) so the
 * app still has content to display.
 */
...
export interface Product {
  ...
  warningsEn: string;
  images: string[];
  pharmacistNoteAr: string | null;
  pharmacistNoteEn: string | null;
  tags: string[];
}
...
export interface Order {
  ...
  date: string;
  total: number;
  items: { productId: string; nameAr: string; nameEn: string; quantity: number; price: number }[];
  requiresPrescription: boolean;
...
export const banners: Banner[] = [
  {
    ...
    gradient: ['#8B2D6B', '#D4528A'], icon: 'tag',
  },
  {
    id: 'banner-2',
    ...
    gradient: ['#059669', '#34D399'], icon: 'shieldCheck',
  },
  {
    id: 'banner-3',
    ...
    gradient: ['#5C1D47', '#8B2D6B'], icon: 'truck',
  },
];
